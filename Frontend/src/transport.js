...
const Transport = () => {
    ...
    useEffect(() => {
        if (location.lat && location.lng) {
            ...
            fetch(`http://localhost:4000/api/transport-facilities?lat=${location.lat}&lng=${location.lng}`)
                .then(response => response.json())
                .then(data => {
                    console.log("Transport facilities data:", data);
                    setTransportFacilities(data);
                })
                .catch(error => console.error("Error fetching transport facilities", error));
        }
    }, [location]);
    ...
    return (
        <div>
            ...
            <div className="mt-20 container">
                <h1 className="text-4xl font-bold text-center mb-8 fade-in text-green-700">Transport Facilities</h1>
                    ...
                    <div>
                        ...
                        <div className="facilities-list">
                            {transportFacilities.length > 0 ? (
                                transportFacilities.map((facility, index) => (
                                    <div key={index} className="card slide-up">
                                        ...
                                        <p className="text-gray-700">Vehicle: {facility.vehicle}</p>
                                        <p className="text-gray-700">Address: {facility.address}</p>
                                    </div>
                                ))
                            ) : (
                                <p className="text-center text-gray-700">No transport facilities found nearby.</p>
                            )}
                        </div>
                    </div>
                ) : (
